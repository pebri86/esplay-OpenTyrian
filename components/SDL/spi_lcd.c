...
void IRAM_ATTR displayTask(void *arg)
{
    ...
    ret = spi_bus_initialize(/*CONFIG_HW_LCD_MISO_GPIO == 19 ? VSPI_HOST :*/ VSPI_HOST, &buscfg, 2); // DMA Channel
    assert(ret == ESP_OK);
    //Attach the LCD to the SPI bus
    ret = spi_bus_add_device(/*CONFIG_HW_LCD_MISO_GPIO == 19 ? VSPI_HOST :*/ VSPI_HOST, &devcfg, &spi);
    assert(ret == ESP_OK);
    //Initialize the LCD
    ili_init(spi);
    SDL_UnlockDisplay();
